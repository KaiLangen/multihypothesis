            ...
            for(int y=0;y<iRange;y++)
            {
              for(int x=0;x<iRange;x++)
              {
                imgPel[0]=imgPrevBuffer[(iPosx[0]+2*x)+
                                        (iPosy[0]+2*y)*2*(2*iPadSize+iWidth)];
                imgPel[1]=imgNextBuffer[(iPosx[1]+2*x)+
                                        (iPosy[1]+2*y)*2*(2*iPadSize+iWidth)];
  ...
  for(int iIndex=0;iIndex<iWidth*iHeight/(iRange*iRange);iIndex++)
  {
    ...
    for(int j=0;j<iRange;j++)
      for(int i=0;i<iRange;i++)
      {
        if(iMode==0)
        {
          px[0]=2*(varCandidate[iIndex].iCx+i+iPadSize)+
                   varCandidate[iIndex].iMvx;
          py[0]=2*(varCandidate[iIndex].iCy+j+iPadSize)+
                   varCandidate[iIndex].iMvy;
          px[1]=2*(varCandidate[iIndex].iCx+i+iPadSize)-
                   varCandidate[iIndex].iMvx;
          py[1]=2*(varCandidate[iIndex].iCy+j+iPadSize)-
                   varCandidate[iIndex].iMvy;
        }
        else
        {
          ...
          py[0]=2*(varCandidate[iIndex].iCy+j+iPadSize)+
                   varCandidate[iIndex].iMvy;
          px[1]=2*(varCandidate2[iIndex].iCx+i+iPadSize)+
                   varCandidate2[iIndex].iMvx;
          py[1]=2*(varCandidate2[iIndex].iCy+j+iPadSize)+
                   varCandidate2[iIndex].iMvy;
        }
        pel[0]=imgPrevBuffer[px[0]+py[0]*2*(2*iPadSize+iWidth)];
        pel[1]=imgNextBuffer[px[1]+py[1]*2*(2*iPadSize+iWidth)];

        int pos=(varCandidate[iIndex].iCx+i)+
                (varCandidate[iIndex].iCy+j)*iWidth;
        if(imgDst!=NULL)
        {
